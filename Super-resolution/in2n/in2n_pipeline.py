...
class InstructNeRF2NeRFPipeline(VanillaPipeline):
    ...
    def __init__(
        ...
        self.ip2p = InstructPix2Pix(self.ip2p_device, ip2p_use_full_precision=True)
    ...
    def get_train_loss_dict(self, step: int):
        ...
        if (step % self.config.edit_rate == 0):
            ...
            for i in range(self.config.edit_count):
                ...
                print(original_image.mean(),original_image.std())
                r = Image.fromarray(((original_image.detach().cpu().squeeze(0).permute(1,2,0).numpy())*255).astype(np.uint8))
                r.save("te.png")
                orig_im = Image.open("te.png")
                #orig_im = torch.tensor(orig_im).unsqueeze(0).permute(0,3,1,2)/255.0 
                print(self.config.prompt,original_image.dtype)
                with autocast():
                    edited_image = self.ip2p.edit_image(
                           self.config.prompt,
                            (2*original_image.to(self.ip2p_device)-1),
